...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollArea && scrollArea.scrollTop > 60) {
        setIsScrolled(true)
      } else {
        setIsScrolled(false)
      }
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 w-full ${
        isScrolled ? "bg-[#2A2F5B]/95 backdrop-blur-sm shadow-md py-2 sm:py-3" : "bg-transparent py-3 sm:py-5"
      }`}
    >
      <div className="container mx-auto px-3 sm:px-4 md:px-6 max-w-7xl">
        <div className="flex items-center justify-between">
          {/* Logo */}
          <Link href="/" className="flex items-center" onClick={() => track("Logo Clicked", { section: "navbar" })}>
            <span className="text-white font-bold text-sm sm:text-base font-montserrat">AI Workshop</span>
          </Link>
